...
void init_schqueue(struct schqueue *q) { memset(q, 0, sizeof(*q)); }

void _acquire_sched_lock() { _acquire_spinlock(&sched_lock); }

void _release_sched_lock() { _release_spinlock(&sched_lock); }
...
void activate_group(struct container *group) {
    ...
    if (parent == group)
        return;
    ASSERT(!_rb_insert(&group->schinfo.sch_node, &parent->schqueue.sch_root,
                       cfs_cmp));
...
// if the proc->state is SLEEPING/UNUSED, set the process state to RUNNABLE, add
// it to the sched queue, and return true if the proc->state is DEEPSLEEING, do
// nothing if onalert or activate it if else, and return the corresponding
// value.
bool _activate_proc(struct proc *p, bool onalert) {
    ...
    if (r) {
        ...
        ASSERT(!_rb_insert(&p->schinfo.sch_node, &parent->schqueue.sch_root,
                           cfs_cmp));
...
static void update_this_state(enum procstate new_state) {
    ...
    for (struct container *group = this->container; group != group->parent;
         group = group->parent) {
        rb_node child = &group->schinfo.sch_node;
        rb_root parent = &group->parent->schqueue.sch_root;

        // if container already in its parent's schqueue, erase and re-insert
        // for maintaining rbtree
    ...
    if (new_state == RUNNABLE) {
        ...
        ASSERT(!_rb_insert(&this->schinfo.sch_node, &parent->schqueue.sch_root,
                           cfs_cmp));
        // if group was empty, activate it
        if (parent->schqueue.sch_cnt++ == 0)
            activate_group(parent);
...
static struct proc *pick_next() {
    ...
    // bottom-up, recursive
    for (struct container *child = next->container, *parent = child->parent;
         --child->schqueue.sch_cnt == 0 && child != parent;
