...
#define increment_rc(checker, rc)                                              \
    checker_begin_ctx_before_call(checker, _increment_rc, rc)

// atomic decrement reference count by one. Returns true if reference
// count becomes zero or below.
#define decrement_rc(checker, rc)                                              \
    checker_end_ctx_after_call(checker, _decrement_rc, rc)
